...
# 2. PAGE CONFIG & ORIGINAL UI STYLING
...
if uploaded_file and not st.session_state.file_ready:
    with st.status("🚀 Processing Knowledge Base...", expanded=True) as status:
        ...
        # FIX: Using the absolute most stable embedding alias
...
# 6. MAIN INTERFACE (Original UI Layout)
if not st.session_state.file_ready:
    ...
    # FIX: Using the "latest" stable Flash alias to avoid 404/429 loops
